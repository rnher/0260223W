...

{
    ...
    public $per_page = 8; // SỐ RECORD TRÊN 1 TRANG
...

    {
        ...
        $count_items = $this->num_row -  $this->per_page *  $this->cur_page;
        if($count_items >= 1){
        if ($next_btn && $this->cur_page < $no_of_paginations) {

            $nex = $this->cur_page + 1;

            $msg .= "<li p='$nex' class='active loadding-extra'><a>Hiển thị thêm $count_items sản phẩm <span><i class='fa fa-angle-down'></i></span></li>";
        } else if ($next_btn) {

            $msg .= "<li class='$this->class_inactive loadding-extra'><a>Hiển thị thêm $count_items sản phẩm <span><i class='fa fa-angle-down'></i></span></a></li>";
        }
