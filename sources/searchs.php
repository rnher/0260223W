<?php
    ...
    if($func->isAjax()){
        ...
        $url = base64_decode($_GET['href']).ltrim($url_page,'&'); //$https_config/san-pham?

        $paging = $func->paginationAdmin($total,$per_page,$page,$url);

        $html['resx'] = $_GET;

        $html['col'] = $func->getTemplateProduct($tintuc);
