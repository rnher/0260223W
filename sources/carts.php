	...
	if($func->isAjax()){
		...
		if($src=='thanh-toan'){
			...
			if(!empty($_POST)){
				...
				$order_random = $func->randString(4);
                ...
                $order_code = 'DHW'.$order_random.'_'.date('dmY');
				                  ...
				                  if(is_array($_SESSION['cart'])){
									...
									for($i=0;$i<$max;$i++){
										...
										$body.='<tr>
											...
											<span>'.$_title.''.$titleColor.''.$titleSize.'</span>
											</td> 
											...
											<td align="right" style="padding:3px 9px" valign="top"><span>'.$func->changeMoney($_price*$_q,$lang).'</span></td>
											</tr>';
			                      	}
			                      	$body.='</tbody>';
