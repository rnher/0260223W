...
<form name="supplier" class="form form-validate"
    action="index.html?com=info&act=save<?php if($_GET['type']!='') echo'&type='. $_GET['type'];?>" method="post"
    ...
    <div class="control_frm">
        <div class="bc">
            <ul id="breadcrumbs" class="breadcrumbs">
                <li><a
                        href="index.html?com=info&act=capnhat<?php if($_GET['type']!='') echo'&type='. $_GET['type'];?>"><span><?=$GLOBAL[$com][$type]['title']?></span></a>
                </li>
    ...
    <div class="<?=($GLOBAL[$com][$type]['full']==true) ? 'oneOne':'colLeft' ?>">
        <div class="widget mtop0">
            ...
            <?php if($GLOBAL[$com][$type]['chuky']){ ?>
            <div class="formRow lang_hidden lang_<?=$k?> <?= ($k == 'vi') ? 'active': '' ?>">
                <label>Chữ ký [<?=$v?>]</label>
                <div class="formRight">
                    <input data-validation="required" data-validation-error-msg="Tên không được để trống" type="text" name="data[chuky_<?=$k?>]" title="Nhập tên danh mục" id="chuky_<?=$k?>"
                        class="tipS validate[required]" value="<?=@$item['chuky_'.$k]?>" />
                </div>
                <div class="clear"></div>
            </div>
            <?php }?>
            <?php if($GLOBAL[$com][$type]['nguoidaidien']){ ?>
            <div class="formRow lang_hidden lang_<?=$k?> <?= ($k == 'vi') ? 'active': '' ?>">
                <label>Người đại diện [<?=$v?>]</label>
                <div class="formRight">
                    <input type="text" name="data[nguoidaidien_<?=$k?>]" title="Nhập người đại diện" id="nguoidaidien_<?=$k?>"
                        class="tipS validate[required]" value="<?=@$item['nguoidaidien_'.$k]?>" />
                </div>
                <div class="clear"></div>
            </div>
            <?php }?>
            <?php if($GLOBAL[$com][$type]['job']){ ?>
            <div class="formRow lang_hidden lang_<?=$k?> <?= ($k == 'vi') ? 'active': '' ?>">
                <label>Vị trí công việc [<?=$v?>]</label>
                <div class="formRight">
                    <input type="text" name="data[job_<?=$k?>]" title="Nhập vị trí" id="job_<?=$k?>"
                        class="tipS validate[required]" value="<?=@$item['job_'.$k]?>" />
                </div>
                <div class="clear"></div>
            </div>
            <?php }?>
