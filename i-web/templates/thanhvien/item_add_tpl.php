...
<div class="wrapper">
    ...
    <form name="frm" class="form" method="post" action="index.html?com=member&act=save&id=<?= $_GET['id'] ?>&type=<?= $_GET['type'] ?>" enctype="multipart/form-data" class="nhaplieu" onSubmit="return js_submit();">
        <div class="oneTwo">
            <div class="widget mtop0">
                ...
                <div class="formRow">
                    <label>#ID :</label>
                    <div class="formRight">
                        <input type="text" name="data[username]" id="username" value="<?= $item['username'] ?>" class="input" <?php if ($_GET['act'] == 'edit') { ?> readonly="readonly" <?php } ?> />
                    </div>
                    <div class="clear"></div>
                </div>
                ...
                <div class="formRow">
                    ...
                    <div class="formRight">
                        <input type="text" name="data[stt]" value="<?= isset($item['stt']) ? $item['stt'] : 1 ?>" style="width:30px">
                    </div>
                    <div class="clear"></div>
                </div>
            </div>
                    ...
                    <div class="formRight">
                        <select name="data[id_city]" id="acc-city">
                            ...
                            <?php foreach ($city_place as $key => $value) { ?>
                                <option value="<?= $value['id'] ?>" <?php if ($userInfo['id_city'] == $value['id']) echo 'selected'; ?>><?= $value['ten'] ?></option>
                            <?php } ?>
                        </select>
                    </div>
                    <div class="clear"></div>
                        ...
                        <select name="data[id_dist]" id="acc-dist">
                            ...
                            <?php foreach ($dataDist as $key => $value) { ?>
                                <?php if ($userInfo['id_dist'] == $value['id']) { ?>
                                    <option value="<?= $value['id'] ?>" <?php if ($userInfo['id_dist'] == $value['id']) echo 'selected'; ?>><?= $value['ten'] ?></option>
                                <?php } ?>
                            <?php } ?>
                        ...
                        <select name="data[id_ward]" id="acc-ward">
                            ...
                            <?php foreach ($dataWard as $key => $value) { ?>
                                <?php if ($item['id_ward'] == $value['id']) { ?>
                                    <option value="<?= $value['id'] ?>" <?php if ($userInfo['id_ward'] == $value['id']) echo 'selected'; ?>><?= $value['ten'] ?></option>
                                <?php } ?>
                            <?php } ?>
                    ...
                    <div class="formRight">
                        <div class="box-admin" style="display:flex; align-items:center;justify-content:flex-end">
                            <div class="box-action">
                                ...
                                <a class="btn btn-sm bg-gradient-danger text-white" href="index.html?com=member&act=man&type=<?= $_GET['type'] ?>">
