...
if (isset($idl) && !empty($idl)) {
    $c2 = $db->rawQuery("select *, photo, id,ten_$lang as ten,tenkhongdau_$lang as tenkhongdau from #_baiviet_cat where hienthi=1 and type=? and id_list= ?  order by stt asc,id desc", array("san-pham", $idl));
    $sp = $db->rawQueryOne("select * from #_baiviet_list where id=? and hienthi=1 and type=? order by stt desc ", array($idl, 'san-pham'));
} else if (isset($idc)) {
    $sp = $db->rawQueryOne("select * from #_baiviet_cat where id=? and hienthi=1 and type=? order by stt desc ", array($idc, 'san-pham'));
}
                                ...
                                <script>
                                    $(() => {
                                        ...
                                        _FRAMEWORK.pagingData(1, <?= $row_setting['page_in'] ?>, "ajax_paging.php",
                                            ...
                                            <?= (empty($idl) ? 0 : $idl) ?>, <?= (empty($idc) ? 0 : $idc) ?>, 'san-pham', '#view-product-list-1');
                                    });
                                </script>
